...
class RealEmailService {
  ...
  // Envoie un email via Mailjet (via proxy backend pour éviter CORS)
  async sendWithMailjetAPI(emailData, emailConfig, type) {
    try {
      ...
      console.log('📧 [Mailjet via Proxy] Envoi en cours...', {
        to: emailData.to,
        subject: emailData.subject,
      ...
      const possibleEndpoints = [
        '/api/send-email',           // Vercel/API Routes
        '/.netlify/functions/send-email', // Netlify Functions
        '/api/send-email.js',        // Vercel Functions
        emailConfig.apiEndpoint || '/api/send-email' // Endpoint personnalisé
      ];
      ...
      for (const endpoint of possibleEndpoints) {
        try {
          ...
          console.log(`✅ [Mailjet via ${endpoint}] Email envoyé avec succès:`, result);

          return {
            success: true,
            messageId: result.messageId,
            message: `Email envoyé avec succès via Mailjet (${endpoint})`,
            provider: 'mailjet',
            response: result
          };

        } catch (error) {
          console.warn(`❌ Échec avec ${endpoint}:`, error.message);
          lastError = error;
          continue;
        }
      }
      ...
      throw new Error(`Tous les endpoints ont échoué. Dernier erreur: ${lastError.message}. 
        Vérifiez que votre API backend est déployée ou utilisez EmailJS.`);

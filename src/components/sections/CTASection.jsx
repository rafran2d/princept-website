...
const CTASection = ({ section, useGlobalStyles }) => {
  ...
  // Vérifier si la section CTA est vide
  const hasTitle = title && String(title).trim() !== '';
  const hasDescription = description && String(description).trim() !== '';
  const hasButtonText = buttonText && String(buttonText).trim() !== '';
  const hasButtonLink = buttonLink && String(buttonLink).trim() !== '';
  const isEmpty = !hasTitle && !hasDescription && !hasButtonText && !hasButtonLink;

...
const ContactSection = ({ section, useGlobalStyles }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Choisir le service selon le provider et l'environnement
      const shouldUseRealService = emailConfig.provider === 'emailjs' || 
                                   emailConfig.provider === 'mailjet' || 
                                   emailConfig.provider === 'api';
      
      const selectedService = shouldUseRealService ? realEmailService : emailService;
      const validationErrors = selectedService.validateEmailConfig(emailConfig);
      
      if (validationErrors.length > 0) {
        throw new Error(`Configuration email incomplète: ${validationErrors.join(', ')}`);
      }

      // Envoi des emails (réel ou simulation)
      const result = await selectedService.sendContactEmails(formData, emailConfig, settings);
      
      console.log(`📧 Service utilisé: ${shouldUseRealService ? 'RÉEL' : 'SIMULATION'}`, {
      ...
      setSubmitError(error.message || 'Une erreur est survenue lors de l\'envoi du message. Veuillez réessayer.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const getCurrentOpenStatus = (officeHours) => {
    ...
    if (!todaySchedule) {
      return { isOpen: false, message: 'Nous sommes fermés aujourd\'hui' };
    }
    ...
    if (todaySchedule.isHoliday) {
      if (todaySchedule.holidayOpen && todaySchedule.holidayClose) {
        ...
        if (currentTime >= openTime && currentTime < closeTime) {
          return { isOpen: true, message: `Ouvert aujourd'hui (${todaySchedule.holidayName}) jusqu'à ${todaySchedule.holidayClose}` };
        } else {
          return { isOpen: false, message: `Fermé - ${todaySchedule.holidayName}` };
        }
      } else {
    ...
    if (todaySchedule.open && todaySchedule.close) {
      ...
      if (currentTime >= openTime && currentTime < closeTime) {
        return { isOpen: true, message: `Nous sommes ouverts jusqu'à ${todaySchedule.close}` };
      } else if (currentTime < openTime) {
        return { isOpen: false, message: `Nous ouvrons à ${todaySchedule.open}` };
      } else {
        return { isOpen: false, message: 'Nous sommes fermés pour aujourd\'hui' };
      }
    }
                ...
                <div className="group">
                  <div className="flex items-center mb-4">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-500">Nos horaires d'ouverture</p>
                    </div>
                  </div>
                  ...
                  <div className="bg-gradient-to-r from-gray-50 to-gray-100 rounded-2xl p-6 shadow-inner">
                    ...
                    <div className="mt-4 pt-4 border-t border-gray-200">
                      <div className="flex items-center justify-center space-x-2">
                        {(() => {
                          const status = getCurrentOpenStatus(settings.officeHours);
                          return (
                            <>

...
const GallerySectionEditable = ({ section, useGlobalStyles, isEditing = false }) => {
  ...
  // Helper pour gérer les valeurs multilingues
  const getLocalizedValue = (value) => {
    if (typeof value === 'string') return value;
    if (typeof value === 'object' && value !== null) {
      return value[currentLangCode] || value['fr'] || value['en'] || Object.values(value)[0] || '';
    }
    return '';
  };
  ...
  const shouldUseSlider = displayImages.length > 4 && !isEditing;
  ...
  // Auto-play pour le slider (désactivé en mode édition)
  useEffect(() => {
    if (!shouldUseSlider || !isAutoPlaying || totalSlides <= 1 || isEditing) return;

    const interval = setInterval(nextSlide, 5000);
    return () => clearInterval(interval);
  }, [shouldUseSlider, isAutoPlaying, nextSlide, totalSlides, isEditing]);
            ...
            {isEditing ? (
              <>
                <EditableTitle
                  sectionId={section.id}
                  fieldPath="title"
                  className="agency-gallery-title"
            ...
            <div className="agency-gallery-grid">
              ...
              return (
                ...
                >
                  ...
                  <h4 className="agency-gallery-card-title">{getLocalizedValue(image.alt)}</h4>
                  {image.link && (
          ...
          <div className={`grid md:grid-cols-2 lg:grid-cols-${isTheme('slack') ? '3' : '3'} gap-6`}>
            {displayImages.map((image) => {
              ...
              return (
                <ImageWrapper
                  key={image.id}
                  className="relative group overflow-hidden rounded-xl shadow-lg hover:shadow-2xl transition-all duration-300 transform hover:scale-105 cursor-pointer flex items-center justify-center"
                  style={{ 
                    minHeight: '16rem',
                ...
                >
                  ...
                  <div className="absolute bottom-4 left-4 right-4 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                    <h4 className="text-white font-medium">{getLocalizedValue(image.alt)}</h4>
                  </div>

                  {image.link && (
                    <div className="absolute top-4 right-4 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                      <div className="bg-white bg-opacity-90 text-gray-800 text-xs px-2 py-1 rounded-full">
...
export default GallerySectionEditable;